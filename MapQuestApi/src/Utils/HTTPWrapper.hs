...
exceptions.
-}
module Utils.HTTPWrapper (download) where
...
download :: String  -- ^ URL to request
...
downloadURL url = do
        result <-  E.try (HTTP.simpleHTTP (HTTP.getRequest url))  :: IO (Either E.SomeException (Stream.Result (Response String)))
        case result of
          ...
          case resp of
           ...
           Right r ->
            case rspCode r of
             ...
             (3,_,_) ->
               case findHeader Headers.HdrLocation r of --redirect
